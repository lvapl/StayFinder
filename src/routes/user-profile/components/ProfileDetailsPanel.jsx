    ...
    <div className="bg-white shadow sm:rounded-lg flex flex-col">
      <div className="px-4 py-5 sm:px-6">
        ...
        <p className="mt-1 max-w-2xl text-gray-500">
          Сохраняйте актуальность ваших данных для лучшего обслуживания
        </p>
      </div>
      <div className="border-t border-gray-200">
        <dl>
          {isEditMode ? (
            ...
            // Display fields
            <>
              <DisplayField label="Имя" value={firstName} />
              <DisplayField label="Фамилия" value={lastName} />
              <DisplayField
                label="Email"
                value={email}
              ...
              <DisplayField
                label="Телефон"
                value={phoneNumber || 'Добавьте номер телефона'}
                verified={isPhoneVerified}
              />
              <DisplayField
                label="Дата рождения"
                value={dateOfBirth || 'Добавьте дату рождения'}
              />
              <DisplayField label="Страна" value={nationality} />
            </>
          )}
        </dl>
      </div>
...
const DisplayField = ({ label, value, verified }) => (
  ...
  >
    ...
    <dd className="mt-1 text-gray-900 sm:mt-0 sm:col-span-2">
      {value}{' '}
      {verified && <span className="text-green-500 font-medium">Подтверждено</span>}
    </dd>
  </div>
);
