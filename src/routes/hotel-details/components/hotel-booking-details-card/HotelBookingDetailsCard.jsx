...
const HotelBookingDetailsCard = ({ hotelCode }) => {
  ...
  const [selectedRoom, setSelectedRoom] = useState({
    value: '1 Стандартная кровать размера King',
    label: '1 Стандартная кровать размера King',
  });
  ...
  const guestOptions = Array.from(
    ...
    (_, i) => ({ value: i + 1, label: `${i + 1} ${i === 0 ? 'гость' : i < 4 ? 'гостя' : 'гостей'}` })
  );
  const roomNumberOptions = Array.from(
    { length: bookingDetails.maxRoomsAllowedPerGuest },
    (_, i) => ({ value: i + 1, label: `${i + 1} ${i === 0 ? 'комната' : 'комнаты'}` })
  );
  const roomOptions = [
    {
      value: '1 Стандартная кровать размера King',
  ...
  const calculatePrices = () => {
    ...
    if (!isNaN(totalPrice)) {
      setTotal(`${formatPrice(totalPrice)} р.`);
    }
    setTaxes(`${formatPrice(totalGst)} р.`);
  };
  ...
  return (
    <div className="mx-2 bg-white shadow-xl rounded-xl overflow-hidden mt-2 md:mt-0 w-full md:w-[380px]">
      <div className="px-6 py-4 bg-blue-600 text-white">
        <h2 className="text-xl font-bold">Информация о бронировании</h2>
      </div>
      <div className="p-6 text-sm md:text-base">
        ...
        {/* Dates & Time */}
        <div className="mb-4">
          <div className="font-semibold text-gray-800">Даты и время</div>
        ...
        {/* Reservation */}
        <div className="mb-4">
          <div className="font-semibold text-gray-800">Бронирование</div>
          <Select
            value={selectedRooms}
        ...
        {/* Room Type */}
        <div className="mb-4">
          <div className="font-semibold text-gray-800">Тип номера</div>
          <Select
            value={selectedRoom}
        ...
        {/* Per day rate */}
        <div className="mb-4">
          <div className="font-semibold text-gray-800">Стоимость за день</div>
          <div className="text-gray-600">
            {formatPrice(bookingDetails.currentNightRate)} р.
          </div>
        </div>
      ...
      <div className="px-6 py-4 bg-gray-50">
        <button
          ...
          className="w-full bg-amber-500 text-white py-2 rounded hover:bg-yellow-600 transition duration-300"
        >
